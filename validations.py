...
def validate_rea_eq10_from_tout(
    ...
    """Diagnóstico simple de las Tablas 10/11 usando exactamente la Ec. (10).

    La prueba NO calibra nada. Para cada mes usa Tin, Tamb, DNI y mdot publicados,
    ejecuta el PTC, toma solamente ``Tout`` del modelo y reconstruye después la
    eficiencia con la misma base documental de Rea Quille::

        eta = mdot * Cp * (Tout - Tin) / (Aa * DNI)

    También recalcula la eficiencia de referencia a partir de ``Tout_ref`` para
    verificar que la Tabla 10/11 es internamente consistente con la Ec. (10).
    Si se suministra ``calibration``, sus parámetros identificados se aplican a
    cada mes antes de simular; la función sigue siendo sólo una prueba, no vuelve
    a optimizar.
    """
    city_key = city.strip().lower()
    if city_key.startswith("foz"):
    ...
    for i in range(12):
        ...
        if calibration_table is not None:
            ...
            _apply_inverse_parameters(cfg, db, parameter_ids, values)
        ...
        eta_python_dni_basis = float(result.scalar_diag.get("eta_dni_basis_pct", result.scalar_diag["eta_pct"])[k])
    ...
    metrics = {
        ...
        "max_reference_eq10_mismatch_pp": float(np.max(np.abs(table["Delta_ref_publicada_vs_Eq10_pp"]))),
        "max_python_native_eq10_mismatch_pp": float(np.max(np.abs(table["Delta_Python_interna_vs_Eq10_pp"]))),
    }
