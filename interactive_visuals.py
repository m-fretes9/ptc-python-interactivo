...
<style>
  ...
  *{{box-sizing:border-box}}
  ...
  .absorber-base{{fill:none;stroke:#111827;stroke-width:9}}
...
<div class="card">
  ...
  <div class="note">El “mapa de calor” representa la <b>distribución óptica estimada de la potencia absorbida</b> alrededor de la circunferencia externa del absorbedor a partir de ray tracing ideal. No es un campo térmico CFD ni incluye conducción circunferencial dentro de la pared.</div>
</div>
<script>
...
function renderHeat(){{
  ...
  for(let b=0;b<bins;b++){{const a0=b/bins*Math.PI*2,a1=(b+1)/bins*Math.PI*2,t=counts[b]/mx,p=el('path',{{d:arcPath(c[0],c[1],r,a0,a1),fill:'none',stroke:colorScale(t),'stroke-width':12,'stroke-linecap':'butt'}});p.appendChild(el('title'));p.firstChild.textContent=`Sector ${{b+1}} · intensidad relativa ${{(100*t).toFixed(1)}}%`;arcs.appendChild(p);}}
