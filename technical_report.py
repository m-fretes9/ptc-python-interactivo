...
def build_technical_report(config: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Transicion de regimen: laminar hasta Re = {model.get('Re_laminar_max', 2300):.6g}; turbulento desde Re = {model.get('Re_turbulent_min', 4000):.6g}",
        f"Entrada termica: Lth = {model.get('thermal_entrance_factor', 0.05):.6g}*Re*Pr*D2",
        "",
        "MODELO SOLAR",
        f"Modo = {solar['mode']}",
        ...
        "Re = 4*mdot/(pi*D2*mu)",
        "Pr = Cp*mu/k",
        "Entrada térmica laminar: Gz_x = Re*Pr*D2/x",
        "Hausen medio: Nu_bar(x) = 4.36 + 0.0668*Gz_x/[1 + 0.04*Gz_x^(2/3)]",
        "Volumen i: Nu_i = [x_e*Nu_bar(x_e) - x_w*Nu_bar(x_w)]/(x_e-x_w) mientras x < Lth",
        "Térmicamente desarrollado: Nu = 4.36 para volúmenes ubicados aguas abajo de Lth",
        "Dittus-Boelter: Nu = 0.023*Re^0.8*Pr^0.4",
        "Zona transicional: interpolacion smoothstep continua entre la rama laminar axial y la correlacion turbulenta configurada.",
        ...
        "eta_HTF = 100*Q_util/Q_incidente_apertura",
        "eta_optica_abs = 100*Qsolar_abs/Q_incidente_apertura",
        "eta_balance = 100*(Qsolar_abs + Qsolar_vid - Q_perdidas)/Q_incidente_apertura",
        "Q_almacenamiento_estimado = Qsolar_abs + Qsolar_vid - Q_perdidas - Q_util",
    ]
    return "\n".join(lines)
...
def result_summary(result: SimulationResult) -> str:
    ...
    lines = [
        ...
        f"Eficiencia HTF: {result.scalar_diag['eta_pct'][k]:.3f} %",
        f"Eficiencia optica al absorbedor: {result.scalar_diag.get('eta_optical_abs_pct', result.scalar_diag['eta_pct'])[k]:.3f} %",
        f"Eficiencia de balance: {result.scalar_diag.get('eta_balance_pct', result.scalar_diag['eta_pct'])[k]:.3f} %",
        f"Q almacenamiento estimado: {result.scalar_diag.get('Qstorage_est_W', result.scalar_diag['Quseful_W']*0.0)[k]:.3f} W",
